...
describe('Voronoi', () => {
  it('voronoi.renderCell(i, context) is a noop for coincident points', () => {
    ...
    expect(voronoi.renderCell(3, {} as any)).toBeUndefined()
  })

  it('voronoi.renderCell(i, context) handles midpoint coincident with circumcenter', () => {
    const voronoi = Delaunay.from([[0, 0], [1, 0], [0, 1]]).voronoi([-1, -1, 2, 2])
    const context = new Context()
    voronoi.renderCell(0, context as any)
    expect(context.toString()).toBe('M-1,-1L0.5,-1L0.5,0.5L-1,0.5Z')
    voronoi.renderCell(1, context as any)
    expect(context.toString()).toBe('M2,-1L2,2L0.5,0.5L0.5,-1Z')
    voronoi.renderCell(2, context as any)
    expect(context.toString()).toBe('M-1,2L-1,0.5L0.5,0.5L2,2Z')
  })

  it('voronoi.contains(i, x, y) is false for coincident points', () => {
    const voronoi = Delaunay.from([[0, 0], [1, 0], [0, 1], [1, 0]]).voronoi([-1, -1, 2, 2])
    expect(voronoi.contains(3, 1, 0)).toBe(false)
    expect(voronoi.contains(1, 1, 0)).toBe(true)
  })
  ...
  it('voronoi.update() updates the voronoi', () => {
    ...
    expect(p).toEqual([[-500, 500], [-500, -140], [-240, -140], [-140, 60], [-140, 500], [-500, 500]])
  })

  it('voronoi.update() updates a degenerate voronoi', () => {
    const pts = [10, 10, -290, 10, 10, -290, -290, -290, -90, -90]
    const delaunay = new Delaunay(Array.from({ length: pts.length }).fill(0) as number[])
    const voronoi = delaunay.voronoi([-500, -500, 500, 500])
    expect(voronoi.cellPolygon(0)).toEqual([[500, -500], [500, 500], [-500, 500], [-500, -500], [500, -500]])
    expect(voronoi.cellPolygon(1)).toBe(null)
    for (let i = 0; i < delaunay.points.length; i++) {
  ...
  it('zero-length edges are removed', () => {
    ...
    expect(voronoi1.cellPolygon(0)!.length).toBe(4)
    const voronoi2 = Delaunay.from([[10, 10], [20, 10]]).voronoi([0, 0, 30, 20])
    expect(voronoi2.cellPolygon(0)).toEqual([[0, 20], [0, 0], [15, 0], [15, 20], [0, 20]])
  })
  ...
  it('voronoi neighbors are clipped', () => {
    ...
    expect([0, 1, 2, 3, 4, 5].map(i => [...voronoi.neighbors(i)].sort())).toEqual([[1, 2], [0, 2, 3], [0, 1, 4], [1], [2], []])
  })
  ...
  it('unnecessary points on the corners are avoided (#88)', () => {
    for (const [points, lengths] of [
      ...
      expect([...voronoi.cellPolygons()].map(d => d.length)).toEqual(lengths)
    }
  })

  it('a degenerate triangle is avoided', () => {
    const pts: [number, number][] = [[424.75, 253.75], [424.75, 253.74999999999997], [407.17640687119285, 296.17640687119285], [364.75, 313.75], [322.32359312880715, 296.17640687119285], [304.75, 253.75], [322.32359312880715, 211.32359312880715], [364.75, 193.75], [407.17640687119285, 211.32359312880715], [624.75, 253.75], [607.1764068711929, 296.17640687119285], [564.75, 313.75], [522.3235931288071, 296.17640687119285], [504.75, 253.75], [564.75, 193.75]]
    const voronoi = Delaunay.from(pts).voronoi([10, 10, 960, 500])
    expect(voronoi.cellPolygon(0)!.length).toBe(4)
  })
  ...
  it('cellPolygons filter out empty cells and have the cell index as a property', () => {
    ...
    expect([...voronoi.cellPolygons()]).toEqual([
      Object.assign([[0, 0], [1, 0], [0, 1], [0, 0]] as [number, number][], { index: 0 }),
      Object.assign([[2, 2], [0, 2], [0, 1], [1, 0], [2, 0], [2, 2]] as [number, number][], { index: 2 }),
    ])
  })
  ...
  it('voronoi.neighbors returns the correct neighbors', () => {
    ...
    expect([0, 1, 2, 3].map(i => [...voronoi.neighbors(i)].sort())).toEqual([[1], [0, 2, 3], [1, 3], [1, 2]])
  })
  ...
  it('voronoi returns the expected result (#136)', () => {
    ...
    expect(voronoi.cellPolygon(3)!.length).toBe(6)
  })

  it('voronoi returns the expected result (#141)', () => {
    const points = [[10, 190]].concat(Array.from({ length: 7 }, (_d, i) => [i * 80, (i * 50) / 7]))
    const voronoi = Delaunay.from(points).voronoi([1, 1, 499, 199])
    expect(Array.from(voronoi.cellPolygons(), (d) => d.length)).toEqual([7, 5, 5, 5, 6, 5, 5, 5])
  })
  ...
  it('voronoi returns the expected result for collinear points (#146)', () => {
    ...
    expect(Array.from(voronoi.cellPolygons(), (d) => d.map((u) => u.map((d) => Math.round(d))))).toEqual([[[0, 40], [0, 0], [50, 0], [50, 40], [0, 40]], [[100, 0], [100, 40], [50, 40], [50, 0], [100, 0]], [[150, 0], [150, 40], [100, 40], [100, 0], [150, 0]], [[150, 40], [150, 0], [200, 0], [200, 40], [150, 40]], [[250, 0], [250, 40], [200, 40], [200, 0], [250, 0]]])
  })
})
