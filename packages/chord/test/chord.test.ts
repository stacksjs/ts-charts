...
import { chord } from '../src/index.ts'
import { assertInDelta } from './asserts.ts'
...
describe('chord', () => {
  it('chord() has the expected defaults', () => {
    const c = chord()
    expect(c.padAngle()).toBe(0)
    expect(c.sortGroups()).toBe(null)
    expect(c.sortSubgroups()).toBe(null)
    expect(c.sortChords()).toBe(null)
  ...
  it('chord.padAngle(angle) sets the pad angle', () => {
    const c = chord().sortSubgroups(function (a: number, b: number) { return b - a })
    expect(c.padAngle(0.05)).toBe(c)
    expect(c.padAngle()).toBe(0.05)
    const chords = c(matrix)
    assertInDelta(chords.groups, [
