...
import { symbol, symbolAsterisk, symbolCircle, symbolCross, symbolDiamond, symbolDiamond2, symbolPlus, symbolSquare, symbolSquare2, symbolStar, symbolTriangle, symbolTriangle2, symbolWye, symbolTimes } from '../src/index.ts'
import { assertInDelta, assertPathEqual } from './asserts.ts'
import { polygonContext } from './polygonContext.ts'
...
it('symbol().size(f)(...) propagates the context and arguments to the specified function', () => {
  ...
  let actual: any
  symbol().size(function (this: any) { actual = { that: this, args: [].slice.call(arguments) }; return 64 }).apply(expected.that, expected.args)
...
it('symbol().type(f)(...) propagates the context and arguments to the specified function', () => {
  const expected = { that: {}, args: [42] }
  let actual: any
  symbol().type(function (this: any) { actual = { that: this, args: [].slice.call(arguments) }; return symbolCircle }).apply(expected.that, expected.args)
  expect(actual).toEqual(expected)
})

it('symbol.size(size) observes the specified size function', () => {
  const size = function (d: any, i: any) { return d.z * 2 + i }
  const s = symbol().size(size)
...
it('symbol.type(symbolAsterisk) generates the expected path', () => {
  const s = symbol().type(symbolAsterisk).size(function (d: any) { return d })
...
it('symbol.type(symbolCircle) generates the expected path', () => {
  const s = symbol().type(symbolCircle).size(function (d: any) { return d })
...
it('symbol.type(symbolCross) generates the expected path', () => {
  const s = symbol().type(symbolCross).size(function (d: any) { return d })
...
it('symbol.type(symbolDiamond) generates the expected path', () => {
  const s = symbol().type(symbolDiamond).size(function (d: any) { return d })
...
it('symbol.type(symbolDiamond2) generates the expected path', () => {
  const s = symbol().type(symbolDiamond2).size(function (d: any) { return d })
  assertPathEqual(s(0), 'M0,0L0,0L0,0L0,0Z')
  assertPathEqual(s(20), 'M0,-2.800675L2.800675,0L0,2.800675L-2.800675,0Z')
})

it('symbol.type(symbolPlus) generates the expected path', () => {
  const s = symbol().type(symbolPlus).size(function (d: any) { return d })
...
it('symbol.type(symbolStar) generates the expected path', () => {
  const s = symbol().type(symbolStar).size(function (d: any) { return d })
...
it('symbol.type(symbolSquare) generates the expected path', () => {
  const s = symbol().type(symbolSquare).size(function (d: any) { return d })
...
it('symbol.type(symbolSquare2) generates the expected path', () => {
  const s = symbol().type(symbolSquare2).size(function (d: any) { return d })
  assertPathEqual(s(0), 'M0,0L0,0L0,0L0,0Z')
...
it('symbol.type(symbolTriangle) generates the expected path', () => {
  const s = symbol().type(symbolTriangle).size(function (d: any) { return d })
...
it('symbol.type(symbolTriangle2) generates the expected path', () => {
  const s = symbol().type(symbolTriangle2).size(function (d: any) { return d })
  assertPathEqual(s(0), 'M0,0L0,0L0,0Z')
...
it('symbol.type(symbolWye) generates the expected path', () => {
  const s = symbol().type(symbolWye).size(function (d: any) { return d })
...
it('symbol.type(symbolTimes) generates the expected path', () => {
  const s = symbol().type(symbolTimes).size(function (d: any) { return d })
  assertPathEqual(s(0), 'M0,0L0,0M0,0L0,0')
