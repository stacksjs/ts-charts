...
import { area, line, curveMonotoneY } from '../../src/index.ts'
...
it('line.curve(curveMonotoneY)(data) ignores coincident points', () => {
  const l = line().curve(curveMonotoneY)
  const p = l([[0, 200], [50, 200], [100, 100], [150, 0], [200, 0]].map(reflect))
  expect(l([[0, 200], [0, 200], [50, 200], [100, 100], [150, 0], [200, 0]].map(reflect))).toBe(p)
  expect(l([[0, 200], [50, 200], [50, 200], [100, 100], [150, 0], [200, 0]].map(reflect))).toBe(p)
  expect(l([[0, 200], [50, 200], [100, 100], [100, 100], [150, 0], [200, 0]].map(reflect))).toBe(p)
  expect(l([[0, 200], [50, 200], [100, 100], [150, 0], [150, 0], [200, 0]].map(reflect))).toBe(p)
  expect(l([[0, 200], [50, 200], [100, 100], [150, 0], [200, 0], [200, 0]].map(reflect))).toBe(p)
})

it('area.curve(curveMonotoneY)(data) generates the expected path', () => {
  const a = area().curve(curveMonotoneY)
  expect(a(([] as number[][]).map(reflect))).toBeNull()
