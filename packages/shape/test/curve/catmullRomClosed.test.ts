...
import { area, line, curveCatmullRomClosed } from '../../src/index.ts'
...
it('line.curve(curveCatmullRomClosed) uses a default alpha of 0.5 (centripetal)', () => {
  ...
  expect(line().curve(curveCatmullRomClosed)([[0, 1], [1, 3], [2, 1], [3, 3]])).toBe(l([[0, 1], [1, 3], [2, 1], [3, 3]]))
})

it('line.curve(curveCatmullRomClosed.alpha(alpha)) coerces the specified alpha to a number', () => {
  const l = line().curve(curveCatmullRomClosed.alpha('0.5' as any))
  expect(line().curve(curveCatmullRomClosed.alpha(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]])).toBe(l([[0, 1], [1, 3], [2, 1], [3, 3]]))
})

it('area.curve(curveCatmullRomClosed.alpha(alpha)) coerces the specified alpha to a number', () => {
  const a = area().curve(curveCatmullRomClosed.alpha('0.5' as any))
  expect(area().curve(curveCatmullRomClosed.alpha(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]])).toBe(a([[0, 1], [1, 3], [2, 1], [3, 3]]))
})
