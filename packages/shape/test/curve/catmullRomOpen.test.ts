...
import { area, line, curveCatmullRomOpen } from '../../src/index.ts'
...
it('line.curve(curveCatmullRomOpen) uses a default alpha of 0.5 (centripetal)', () => {
  ...
  expect(line().curve(curveCatmullRomOpen)([[0, 1], [1, 3], [2, 1], [3, 3]])).toBe(l([[0, 1], [1, 3], [2, 1], [3, 3]]))
})

it('line.curve(curveCatmullRomOpen.alpha(alpha)) coerces the specified alpha to a number', () => {
  const l = line().curve(curveCatmullRomOpen.alpha('0.5' as any))
  expect(line().curve(curveCatmullRomOpen.alpha(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]])).toBe(l([[0, 1], [1, 3], [2, 1], [3, 3]]))
})

it('area.curve(curveCatmullRomOpen.alpha(0.5))(data) generates the expected path', () => {
  const a = area().curve(curveCatmullRomOpen)
  expect(a([])).toBeNull()
...
it('area.curve(curveCatmullRomOpen) uses a default alpha of 0.5 (centripetal)', () => {
  const a = area().curve(curveCatmullRomOpen)
  expect(area().curve(curveCatmullRomOpen)([[0, 1], [1, 3], [2, 1], [3, 3]])).toBe(a([[0, 1], [1, 3], [2, 1], [3, 3]]))
})

it('area.curve(curveCatmullRomOpen.alpha(alpha)) coerces the specified alpha to a number', () => {
  const a = area().curve(curveCatmullRomOpen.alpha('0.5' as any))
  expect(area().curve(curveCatmullRomOpen.alpha(0.5))([[0, 1], [1, 3], [2, 1], [3, 3]])).toBe(a([[0, 1], [1, 3], [2, 1], [3, 3]]))
})
