...
import { geoCentroid, geoCircle } from '../src/index.ts'
...
describe('geoCentroid', () => {
  ...
  it('the centroid of a set of points and their antipodes is ambiguous', () => {
    expect(geoCentroid({ type: 'MultiPoint', coordinates: [[0, 0], [180, 0]] }).every(isNaN)).toBe(true)
    expect(geoCentroid({ type: 'MultiPoint', coordinates: [[0, 0], [90, 0], [180, 0], [-90, 0]] }).every(isNaN)).toBe(true)
    expect(geoCentroid({ type: 'MultiPoint', coordinates: [[0, 0], [0, 90], [180, 0], [0, -90]] }).every(isNaN)).toBe(true)
  })

  it('the centroid of the empty set of points is ambiguous', () => {
    expect(geoCentroid({ type: 'MultiPoint', coordinates: [] }).every(isNaN)).toBe(true)
  })
  ...
  it('the centroid of a great arc from a point to its antipode is ambiguous', () => {
    expect(geoCentroid({ type: 'LineString', coordinates: [[180, 0], [0, 0]] }).every(isNaN)).toBe(true)
    expect(geoCentroid({ type: 'MultiLineString', coordinates: [[[0, -90], [0, 90]]] }).every(isNaN)).toBe(true)
  })
  ...
  it('the centroid of the equator is ambiguous', () => {
    expect(geoCentroid({ type: 'LineString', coordinates: [[0, 0], [120, 0], [-120, 0], [0, 0]] }).every(isNaN)).toBe(true)
  })
  ...
  it('the centroid of a small circle is its center: 5 degrees', () => {
    assertInDelta(geoCentroid(geoCircle().radius(5).center([30, 45])()), [30, 45], 1e-6)
  })

  it('the centroid of a small circle is its center: 135 degrees', () => {
  ...
  it('the centroid of a small circle is its center: South Pole', () => {
    expect(geoCentroid({ type: 'Polygon', coordinates: [range(-180, 180 + 1 / 2, 1).map(function (x) { return [x, -60] })] })[1]).toBe(-90)
  })
  ...
  it('the centroid of a sphere is ambiguous', () => {
    expect(geoCentroid({ type: 'Sphere' }).every(isNaN)).toBe(true)
  })
  ...
  it('the centroid of the sphere and a point is the point', () => {
    expect(geoCentroid({ type: 'GeometryCollection', geometries: [
      { type: 'Sphere' },
      { type: 'Point', coordinates: [0, 0] }
    ] })).toEqual([0, 0])
    expect(geoCentroid({ type: 'GeometryCollection', geometries: [
