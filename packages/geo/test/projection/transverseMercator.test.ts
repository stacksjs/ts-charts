...
import { geoPath, geoTransverseMercator } from '../../src/index.ts'
...
describe('transverseMercator', () => {
  ...
  it('transverseMercator.rotate(...) does not affect the automatic clip extent', () => {
    ...
    assertInDelta(projection.scale(), 15724992.330511674, 1)
    assertInDelta(projection.translate(), [20418843.897824813, 21088401.790971387], 1)
    projection.rotate([0, 95]).fitExtent([[0, 0], [960, 600]], object)
    assertInDelta(projection.scale(), 15724992.330511674, 1)
    assertInDelta(projection.translate(), [20418843.897824813, 47161426.43770847], 1)
  })
})
