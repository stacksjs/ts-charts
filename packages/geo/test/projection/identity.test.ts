...
import { geoIdentity, geoPath } from '../../src/index.ts'
...
describe('identity', () => {
  ...
  it('geoPath(identity) returns the path', () => {
    ...
    expect(path({ type: 'LineString', coordinates: [[0, 0], [10, 10]] })).toBe('M0,0L10,10')
    identity.translate([30, 90]).scale(2).reflectY(true)
    expect(path({ type: 'LineString', coordinates: [[0, 0], [10, 10]] })).toBe('M30,90L50,70')
  })
  ...
  it('geoPath(identity) respects clipExtent', () => {
    ...
    expect(path({ type: 'LineString', coordinates: [[0, 0], [10, 10]] })).toBe('M5,5L10,10')
    identity.translate([30, 90]).scale(2).reflectY(true).clipExtent([[35, 76], [45, 86]])
    expect(path({ type: 'LineString', coordinates: [[0, 0], [10, 10]] })).toBe('M35,85L44,76')
  })
})
