...
import { geoMercator, geoPath } from '../../src/index.ts'
import { assertInDelta, assertPathEqual } from '../helpers/asserts.ts'
...
describe('mercator', () => {
  ...
  it('mercator.rotate(...) does not affect the automatic clip extent', () => {
    ...
    assertInDelta(projection.scale(), 20969742.365692537, 1)
    assertInDelta(projection.translate(), [30139734.76760269, 11371473.949706702], 1)
    projection.rotate([0, 95]).fitExtent([[0, 0], [960, 600]], object)
    assertInDelta(projection.scale(), 35781690.650920525, 1)
    assertInDelta(projection.translate(), [75115911.95344563, 2586046.4116968135], 1)
  })
})
