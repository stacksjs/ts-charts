...
function assertPathEqual(actual: { toString(): string }, expected: string): void {
...
describe('path', () => {
  ...
  it('path.arc(x, y, radius, startAngle, endAngle) throws an error if the radius is negative', () => {
    ...
    expect(() => { p.arc(100, 100, -50, 0, Math.PI / 2) }).toThrow(/negative radius/)
  })
  ...
  it('path.arc(x, y, radius, startAngle, endAngle) appends a single A command if the angle is less than pi', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, Math.PI / 2)
    assertPathEqual(p, 'M150,100A50,50,0,0,1,100,150')
  })

  it('path.arc(x, y, radius, startAngle, endAngle) appends a single A command if the angle is less than tau', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, Math.PI * 1)
    assertPathEqual(p, 'M150,100A50,50,0,1,1,50,100')
  })

  it('path.arc(x, y, radius, startAngle, endAngle) appends two A commands if the angle is greater than tau', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, Math.PI * 2)
    assertPathEqual(p, 'M150,100A50,50,0,1,1,50,100A50,50,0,1,1,150,100')
  })

  it('path.arc(x, y, radius, 0, pi/2, false) draws a small clockwise arc', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, Math.PI / 2, false)
    assertPathEqual(p, 'M150,100A50,50,0,0,1,100,150')
  })

  it('path.arc(x, y, radius, -pi/2, 0, false) draws a small clockwise arc', () => {
    const p = path(); p.moveTo(100, 50); p.arc(100, 100, 50, -Math.PI / 2, 0, false)
    assertPathEqual(p, 'M100,50A50,50,0,0,1,150,100')
  })

  it('path.arc(x, y, radius, 0, epsilon, true) draws an anticlockwise circle', () => {
  ...
  it('path.arc(x, y, radius, 0, epsilon, false) draws nothing', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, 1e-16, false)
    assertPathEqual(p, 'M150,100')
  })

  it('path.arc(x, y, radius, 0, -epsilon, true) draws nothing', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, -1e-16, true)
    assertPathEqual(p, 'M150,100')
  })

  it('path.arc(x, y, radius, 0, -epsilon, false) draws a clockwise circle', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, -1e-16, false)
    assertPathEqual(p, 'M150,100A50,50,0,1,1,50,100A50,50,0,1,1,150,100')
  })

  it('path.arc(x, y, radius, 0, tau, true) draws an anticlockwise circle', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, 2 * Math.PI, true)
    assertPathEqual(p, 'M150,100A50,50,0,1,0,50,100A50,50,0,1,0,150,100')
  })

  it('path.arc(x, y, radius, 0, tau, false) draws a clockwise circle', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, 2 * Math.PI, false)
    assertPathEqual(p, 'M150,100A50,50,0,1,1,50,100A50,50,0,1,1,150,100')
  })

  it('path.arc(x, y, radius, 0, tau + epsilon, true) draws an anticlockwise circle', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, 2 * Math.PI + 1e-13, true)
    assertPathEqual(p, 'M150,100A50,50,0,1,0,50,100A50,50,0,1,0,150,100')
  })

  it('path.arc(x, y, radius, 0, tau - epsilon, false) draws a clockwise circle', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, 2 * Math.PI - 1e-13, false)
    assertPathEqual(p, 'M150,100A50,50,0,1,1,50,100A50,50,0,1,1,150,100')
  })

  it('path.arc(x, y, radius, tau, 0, true) draws an anticlockwise circle', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, 2 * Math.PI, true)
    assertPathEqual(p, 'M150,100A50,50,0,1,0,50,100A50,50,0,1,0,150,100')
  })

  it('path.arc(x, y, radius, tau, 0, false) draws a clockwise circle', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, 2 * Math.PI, false)
    assertPathEqual(p, 'M150,100A50,50,0,1,1,50,100A50,50,0,1,1,150,100')
  })

  it('path.arc(x, y, radius, 0, 13pi/2, false) draws a clockwise circle', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, 13 * Math.PI / 2, false)
    assertPathEqual(p, 'M150,100A50,50,0,1,1,50,100A50,50,0,1,1,150,100')
  })

  it('path.arc(x, y, radius, 13pi/2, 0, false) draws a big clockwise arc', () => {
    const p = path(); p.moveTo(100, 150); p.arc(100, 100, 50, 13 * Math.PI / 2, 0, false)
    assertPathEqual(p, 'M100,150A50,50,0,1,1,150,100')
  })

  it('path.arc(x, y, radius, pi/2, 0, false) draws a big clockwise arc', () => {
    const p = path(); p.moveTo(100, 150); p.arc(100, 100, 50, Math.PI / 2, 0, false)
    assertPathEqual(p, 'M100,150A50,50,0,1,1,150,100')
  })

  it('path.arc(x, y, radius, 3pi/2, 0, false) draws a small clockwise arc', () => {
    const p = path(); p.moveTo(100, 50); p.arc(100, 100, 50, 3 * Math.PI / 2, 0, false)
    assertPathEqual(p, 'M100,50A50,50,0,0,1,150,100')
  })

  it('path.arc(x, y, radius, 15pi/2, 0, false) draws a small clockwise arc', () => {
    const p = path(); p.moveTo(100, 50); p.arc(100, 100, 50, 15 * Math.PI / 2, 0, false)
    assertPathEqual(p, 'M100,50A50,50,0,0,1,150,100')
  })

  it('path.arc(x, y, radius, 0, pi/2, true) draws a big anticlockwise arc', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, Math.PI / 2, true)
    assertPathEqual(p, 'M150,100A50,50,0,1,0,100,150')
  })

  it('path.arc(x, y, radius, -pi/2, 0, true) draws a big anticlockwise arc', () => {
    const p = path(); p.moveTo(100, 50); p.arc(100, 100, 50, -Math.PI / 2, 0, true)
    assertPathEqual(p, 'M100,50A50,50,0,1,0,150,100')
  })

  it('path.arc(x, y, radius, -13pi/2, 0, true) draws a big anticlockwise arc', () => {
  ...
  it('path.arc(x, y, radius, 0, 13pi/2, true) draws a big anticlockwise arc', () => {
    const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, 13 * Math.PI / 2, true)
    assertPathEqual(p, 'M150,100A50,50,0,1,0,100,150')
  })

  it('path.arc(x, y, radius, pi/2, 0, true) draws a small anticlockwise arc', () => {
    const p = path(); p.moveTo(100, 150); p.arc(100, 100, 50, Math.PI / 2, 0, true)
    assertPathEqual(p, 'M100,150A50,50,0,0,0,150,100')
  })

  it('path.arc(x, y, radius, 3pi/2, 0, true) draws a big anticlockwise arc', () => {
    const p = path(); p.moveTo(100, 50); p.arc(100, 100, 50, 3 * Math.PI / 2, 0, true)
    assertPathEqual(p, 'M100,50A50,50,0,1,0,150,100')
  })

  it('path.arc(x, y, radius, pi/2, 0, truthy) draws a small anticlockwise arc', () => {
    for (const trueish of [1, '1', true, 10, '3', 'string'] as const) {
      const p = path(); p.moveTo(100, 150); p.arc(100, 100, 50, Math.PI / 2, 0, trueish as unknown as boolean)
      assertPathEqual(p, 'M100,150A50,50,0,0,0,150,100')
    }
  })

  it('path.arc(x, y, radius, 0, pi/2, falsy) draws a small clockwise arc', () => {
    for (const falseish of [0, null, undefined]) {
      const p = path(); p.moveTo(150, 100); p.arc(100, 100, 50, 0, Math.PI / 2, falseish as unknown as boolean)
      assertPathEqual(p, 'M150,100A50,50,0,0,1,100,150')
    }
  })
  ...
  it('path.arcTo(x1, y1, x2, y2, radius) throws an error if the radius is negative', () => {
    const p = path(); p.moveTo(150, 100)
    expect(() => { p.arcTo(270, 39, 163, 100, -53) }).toThrow(/negative radius/)
  })
