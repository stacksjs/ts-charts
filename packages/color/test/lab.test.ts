...
import { Color, Lab, color, hcl, hsl, lab, rgb } from '../src/index.ts'
import { assertLabEqual, assertRgbApproxEqual } from './asserts.ts'

describe('lab', () => {
  it('lab(...) returns an instance of lab and color', () => {
    const c = lab(120, 40, 50)
    expect(c instanceof Lab).toBe(true)
  ...
  it('lab.toString() converts to RGB and formats as rgb(...) or rgba(...)', () => {
    expect(lab('#abcdef') + '').toBe('rgb(171, 205, 239)')
    expect(lab('moccasin') + '').toBe('rgb(255, 228, 181)')
    expect(lab('hsl(60, 100%, 20%)') + '').toBe('rgb(102, 102, 0)')
    expect(lab('hsla(60, 100%, 20%, 0.4)') + '').toBe('rgba(102, 102, 0, 0.4)')
    expect(lab('rgb(12, 34, 56)') + '').toBe('rgb(12, 34, 56)')
    expect(lab(rgb(12, 34, 56)) + '').toBe('rgb(12, 34, 56)')
    expect(lab(hsl(60, 1, 0.2)) + '').toBe('rgb(102, 102, 0)')
    expect(lab(hsl(60, 1, 0.2, 0.4)) + '').toBe('rgba(102, 102, 0, 0.4)')
  })

  it('lab.toString() reflects l, a and b channel values and opacity', () => {
    const c = lab('#abc')
    c.l += 10; c.a -= 10; c.b += 10; c.opacity = 0.4
    expect(c + '').toBe('rgba(184, 220, 213, 0.4)')
  })

  it('lab.toString() treats undefined channel values as 0', () => {
    expect(lab('invalid') + '').toBe('rgb(0, 0, 0)')
    expect(lab(NaN, 0, 0) + '').toBe('rgb(0, 0, 0)')
    expect(lab(50, NaN, 0) + '').toBe('rgb(119, 119, 119)')
    expect(lab(50, 0, NaN) + '').toBe('rgb(119, 119, 119)')
    expect(lab(50, NaN, NaN) + '').toBe('rgb(119, 119, 119)')
  })
  ...
  it('lab(l, a, b) coerces channel values to numbers', () => {
    assertLabEqual(lab('50' as any, '4' as any, '-5' as any), 50, 4, -5, 1)
  })

  it('lab(l, a, b, opacity) coerces opacity to number', () => {
    assertLabEqual(lab(50, 4, -5, '0.2' as any), 50, 4, -5, 0.2)
  })

  it('lab(l, a, b) allows undefined channel values', () => {
    assertLabEqual(lab(undefined as any, NaN, 'foo' as any), NaN, NaN, NaN, 1)
    assertLabEqual(lab(undefined as any, 4, -5), NaN, 4, -5, 1)
    assertLabEqual(lab(42, undefined as any, -5), 42, NaN, -5, 1)
    assertLabEqual(lab(42, 4, undefined as any), 42, 4, NaN, 1)
  })

  it('lab(l, a, b, opacity) converts undefined opacity to 1', () => {
    assertLabEqual(lab(10, 20, 30, null as any), 10, 20, 30, 1)
  ...
  it('lab(hcl(lab)) does not lose a and b channels if luminance is zero', () => {
  ...
  it('lab(color) converts from another colorspace via rgb()', () => {
    class TestColor extends Color {
      override rgb() { return rgb(12, 34, 56, 0.4) }
      override toString(): string { throw new Error('should use rgb, not toString') }
    }
