...
import { deviation, mean, range, variance } from '@ts-charts/array'
import { randomGamma, randomLcg } from '../src/index.ts'
...
describe('randomGamma', () => {
  it('randomGamma(k) returns random numbers with a mean of k', () => {
    ...
    assertInDelta(mean(range(10000).map(r(10))) as number, 10, 0.05)
  })

  it('randomGamma(k) returns random numbers with a variance of k', () => {
    const r = randomGamma.source(randomLcg(0.6494198931625885))
    assertInDelta(variance(range(10000).map(r(0.1))) as number, 0.1, 0.005)
  ...
  it('randomGamma(k, theta) returns random numbers with a mean of k * theta and a variance of k * theta^2', () => {
    ...
    assertInDelta(mean(range(10000).map(r(1, 2))) as number, 2, 0.05)
    assertInDelta(mean(range(10000).map(r(2, 4))) as number, 8, 0.2)
    assertInDelta(deviation(range(10000).map(r(1, 2))) as number, 2, 0.1)
    assertInDelta(deviation(range(10000).map(r(2, 4))) as number, Math.sqrt(2) * 4, 0.1)
  })
})
