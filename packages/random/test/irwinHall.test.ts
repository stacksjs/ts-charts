...
import { randomIrwinHall, randomLcg } from '../src/index.ts'
...
describe('randomIrwinHall', () => {
  it('randomIrwinHall(n) returns random numbers with a mean of n / 2', () => {
    ...
    assertInDelta(mean(range(10000).map(r(1))) as number, 1 / 2, 0.05)
    assertInDelta(mean(range(10000).map(r(10))) as number, 10 / 2, 0.05)
    assertInDelta(mean(range(10000).map(r(1.5))) as number, 1.5 / 2, 0.05)
    assertInDelta(mean(range(10000).map(r(4.2))) as number, 4.2 / 2, 0.05)
  })
  ...
  it('randomIrwinHall(n) returns random numbers with a variance of n / 12', () => {
    ...
    assertInDelta(variance(range(10000).map(r(10))) as number, 10 / 12, 0.05)
    assertInDelta(variance(range(10000).map(r(1.5))) as number, 1.5 / 12, 0.05)
    assertInDelta(variance(range(10000).map(r(4.2))) as number, 4.2 / 12, 0.05)
  })
