...
jsdomit('transition.select(selector) selects the descendants matching the specified selector, then derives a transition', '<h1 id="one"><child></child></h1><h1 id="two"><child></child></h1>', () => {
...
jsdomit('transition.select(function) selects the descendants returned by the specified function, then derives a transition', '<h1 id="one"><child></child></h1><h1 id="two"><child></child></h1>', () => {
  const one = document.querySelector('#one')!
  const two = document.querySelector('#two')!
  const t1 = selectAll([one, two]).data([1, 2]).transition().delay(function (this: any, d: any) { return d * 10 })
  const t2 = t1.select(function (this: any) { return this.firstChild })
  expect(t2 instanceof transition).toBe(true)
  expect(t2._groups).toEqual([[one.firstChild as Element, two.firstChild as Element]])
  expect(t2._parents).toBe(t1._parents)
  expect(t2._name).toBe(t1._name)
