...
it('linear.invert(y) coerces range values to numbers', () => {
  ...
  expect(scaleLinear().range([new Date(1990, 0, 1), new Date(1991, 0, 1)]).invert(new Date(1990, 6, 2, 13))).toBe(0.5)
})
...
it('linear.domain(domain) coerces domain values to numbers', () => {
  expect(scaleLinear().domain([new Date(1990, 0, 1), new Date(1991, 0, 1)]).domain()).toEqual([631180800000, 662716800000])
  expect(scaleLinear().domain(['0.0', '1.0']).domain()).toEqual([0, 1])
  expect(scaleLinear().domain([new Number(0), new Number(1)]).domain()).toEqual([0, 1])
})
...
it('linear.clamp(clamp) coerces the specified clamp value to a boolean', () => {
  expect(scaleLinear().clamp('true').clamp()).toBe(true)
  expect(scaleLinear().clamp(1).clamp()).toBe(true)
  expect(scaleLinear().clamp('').clamp()).toBe(false)
