...
import { scaleLinear } from '../src/index.ts'
import { roundEpsilon } from './helpers.ts'
...
describe('scaleLinear', () => {
  ...
  it('linear.invert(y) coerces range values to numbers', () => {
    expect(scaleLinear().range(['0', '2'] as any).invert('1' as any)).toBe(0.5)
    const d0 = new Date(1990, 0, 1), d1 = new Date(1991, 0, 1), mid = new Date(+d0 + (+d1 - +d0) / 2)
    expect(scaleLinear().range([d0, d1] as any).invert(mid as any)).toBe(0.5)
  })

  it('linear.invert(y) returns NaN if the range is not coercible to number', () => {
    expect(isNaN(scaleLinear().range(['#000', '#fff']).invert('#999' as any))).toBe(true)
    expect(isNaN(scaleLinear().range([0, '#fff'] as any).invert('#999' as any))).toBe(true)
  })

  it('linear.domain(domain) accepts an array of numbers', () => {
    expect(scaleLinear().domain([]).domain()).toEqual([])
    expect(scaleLinear().domain([1, 0]).domain()).toEqual([1, 0])
    expect(scaleLinear().domain([1, 2, 3]).domain()).toEqual([1, 2, 3])
  })

  it('linear.domain(domain) coerces domain values to numbers', () => {
    expect(scaleLinear().domain([new Date(1990, 0, 1), new Date(1991, 0, 1)] as any).domain()).toEqual([+new Date(1990, 0, 1), +new Date(1991, 0, 1)])
    expect(scaleLinear().domain(['0.0', '1.0'] as any).domain()).toEqual([0, 1])
    expect(scaleLinear().domain([new Number(0), new Number(1)] as any).domain()).toEqual([0, 1])
  })

  it('linear.domain(domain) accepts an iterable', () => {
    expect(scaleLinear().domain(new Set([1, 2])).domain()).toEqual([1, 2])
  })
  ...
  it('linear.range(range) accepts an iterable', () => {
    expect(scaleLinear().range(new Set([1, 2])).range()).toEqual([1, 2])
  })

  it('linear.range(range) can accept range values as colors', () => {
    expect(scaleLinear().range(['red', 'blue'])(0.5)).toBe('rgb(128, 0, 128)')
    expect(scaleLinear().range(['#ff0000', '#0000ff'])(0.5)).toBe('rgb(128, 0, 128)')
    expect(scaleLinear().range(['#f00', '#00f'])(0.5)).toBe('rgb(128, 0, 128)')
    expect(scaleLinear().range(['rgb(255,0,0)', 'hsl(240,100%,50%)'])(0.5)).toBe('rgb(128, 0, 128)')
    expect(scaleLinear().range(['rgb(100%,0%,0%)', 'hsl(240,100%,50%)'])(0.5)).toBe('rgb(128, 0, 128)')
    expect(scaleLinear().range(['hsl(0,100%,50%)', 'hsl(240,100%,50%)'])(0.5)).toBe('rgb(128, 0, 128)')
  })

  it('linear.range(range) can accept range values as arrays or objects', () => {
    expect(scaleLinear().range([{ color: 'red' }, { color: 'blue' }])(0.5)).toEqual({ color: 'rgb(128, 0, 128)' })
    expect(scaleLinear().range([['red'], ['blue']])(0.5)).toEqual(['rgb(128, 0, 128)'])
  })
  ...
  it('linear.rangeRound(range) accepts an iterable', () => {
    expect(scaleLinear().rangeRound(new Set([1, 2])).range()).toEqual([1, 2])
  })
  ...
  it('linear.clamp(clamp) coerces the specified clamp value to a boolean', () => {
    expect(scaleLinear().clamp('true' as any).clamp()).toBe(true)
    expect(scaleLinear().clamp(1 as any).clamp()).toBe(true)
    expect(scaleLinear().clamp('' as any).clamp()).toBe(false)
    expect(scaleLinear().clamp(0 as any).clamp()).toBe(false)
  })
  ...
  it('linear.nice() is an alias for linear.nice(10)', () => {
    expect(scaleLinear().domain([0, 0.96]).nice().domain()).toEqual([0, 1])
    expect(scaleLinear().domain([0, 96]).nice().domain()).toEqual([0, 100])
  })

  it('linear.nice(count) extends the domain to match the desired ticks', () => {
    expect(scaleLinear().domain([0, 0.96]).nice(10).domain()).toEqual([0, 1])
    expect(scaleLinear().domain([0, 96]).nice(10).domain()).toEqual([0, 100])
    expect(scaleLinear().domain([0.96, 0]).nice(10).domain()).toEqual([1, 0])
    expect(scaleLinear().domain([96, 0]).nice(10).domain()).toEqual([100, 0])
    expect(scaleLinear().domain([0, -0.96]).nice(10).domain()).toEqual([0, -1])
    expect(scaleLinear().domain([0, -96]).nice(10).domain()).toEqual([0, -100])
    expect(scaleLinear().domain([-0.96, 0]).nice(10).domain()).toEqual([-1, 0])
    expect(scaleLinear().domain([-96, 0]).nice(10).domain()).toEqual([-100, 0])
    expect(scaleLinear().domain([-0.1, 51.1]).nice(8).domain()).toEqual([-10, 60])
  })

  it('linear.nice(count) nices the domain, extending it to round numbers', () => {
    expect(scaleLinear().domain([1.1, 10.9]).nice(10).domain()).toEqual([1, 11])
    expect(scaleLinear().domain([10.9, 1.1]).nice(10).domain()).toEqual([11, 1])
    expect(scaleLinear().domain([0.7, 11.001]).nice(10).domain()).toEqual([0, 12])
    expect(scaleLinear().domain([123.1, 6.7]).nice(10).domain()).toEqual([130, 0])
    expect(scaleLinear().domain([0, 0.49]).nice(10).domain()).toEqual([0, 0.5])
    expect(scaleLinear().domain([0, 14.1]).nice!(5).domain()).toEqual([0, 20])
    expect(scaleLinear().domain([0, 15]).nice!(5).domain()).toEqual([0, 20])
  })

  it('linear.nice(count) has no effect on degenerate domains', () => {
    expect(scaleLinear().domain([0, 0]).nice(10).domain()).toEqual([0, 0])
    expect(scaleLinear().domain([0.5, 0.5]).nice(10).domain()).toEqual([0.5, 0.5])
  })

  it('linear.nice(count) nicing a polylinear domain only affects the extent', () => {
    expect(scaleLinear().domain([1.1, 1, 2, 3, 10.9]).nice(10).domain()).toEqual([1, 1, 2, 3, 11])
    expect(scaleLinear().domain([123.1, 1, 2, 3, -0.9]).nice(10).domain()).toEqual([130, 1, 2, 3, -10])
  })

  it('linear.nice(count) accepts a tick count to control nicing step', () => {
    expect(scaleLinear().domain([12, 87]).nice!(5).domain()).toEqual([0, 100])
    expect(scaleLinear().domain([12, 87]).nice(10).domain()).toEqual([10, 90])
    expect(scaleLinear().domain([12, 87]).nice(100).domain()).toEqual([12, 87])
  })
  ...
  it('linear.ticks(X) spans linear.nice(X).domain()', () => {
    function check(domain: number[], count: number) {
  ...
  it('linear.tickFormat() is an alias for linear.tickFormat(10)', () => {
    expect(scaleLinear().tickFormat!()(0.2)).toBe('0.2')
    expect(scaleLinear().domain([-100, 100]).tickFormat()(-20)).toBe('\u221220')
  })

  it('linear.tickFormat(count) returns a format suitable for the ticks', () => {
    expect(scaleLinear().tickFormat!(10)(0.2)).toBe('0.2')
    expect(scaleLinear().tickFormat!(20)(0.2)).toBe('0.20')
    expect(scaleLinear().domain([-100, 100]).tickFormat(10)(-20)).toBe('\u221220')
  })

  it('linear.tickFormat(count, specifier) sets the appropriate fixed precision if not specified', () => {
    expect(scaleLinear().tickFormat!(10, '+f')(0.2)).toBe('+0.2')
    expect(scaleLinear().tickFormat!(20, '+f')(0.2)).toBe('+0.20')
    expect(scaleLinear().tickFormat!(10, '+%')(0.2)).toBe('+20%')
    expect(scaleLinear().domain([0.19, 0.21]).tickFormat(10, '+%')(0.2)).toBe('+20.0%')
  })

  it('linear.tickFormat(count, specifier) sets the appropriate round precision if not specified', () => {
    expect(scaleLinear().domain([0, 9]).tickFormat(10, '')(2.10)).toBe('2')
    expect(scaleLinear().domain([0, 9]).tickFormat(100, '')(2.01)).toBe('2')
    expect(scaleLinear().domain([0, 9]).tickFormat(100, '')(2.11)).toBe('2.1')
    expect(scaleLinear().domain([0, 9]).tickFormat(10, 'e')(2.10)).toBe('2e+0')
    expect(scaleLinear().domain([0, 9]).tickFormat(100, 'e')(2.01)).toBe('2.0e+0')
    expect(scaleLinear().domain([0, 9]).tickFormat(100, 'e')(2.11)).toBe('2.1e+0')
    expect(scaleLinear().domain([0, 9]).tickFormat(10, 'g')(2.10)).toBe('2')
    expect(scaleLinear().domain([0, 9]).tickFormat(100, 'g')(2.01)).toBe('2.0')
    expect(scaleLinear().domain([0, 9]).tickFormat(100, 'g')(2.11)).toBe('2.1')
    expect(scaleLinear().domain([0, 9]).tickFormat(10, 'r')(2.10e6)).toBe('2000000')
    expect(scaleLinear().domain([0, 9]).tickFormat(100, 'r')(2.01e6)).toBe('2000000')
    expect(scaleLinear().domain([0, 9]).tickFormat(100, 'r')(2.11e6)).toBe('2100000')
    expect(scaleLinear().domain([0, 0.9]).tickFormat(10, 'p')(0.210)).toBe('20%')
    expect(scaleLinear().domain([0.19, 0.21]).tickFormat(10, 'p')(0.201)).toBe('20.1%')
  })

  it('linear.tickFormat(count, specifier) sets the appropriate prefix precision if not specified', () => {
    expect(scaleLinear().domain([0, 1e6]).tickFormat(10, '$s')(0.51e6)).toBe('$0.5M')
    expect(scaleLinear().domain([0, 1e6]).tickFormat(100, '$s')(0.501e6)).toBe('$0.50M')
  })
  ...
  it('linear.copy() returns a copy with changes to the unknown value are isolated', () => {
    ...
    x.unknown(2)
    expect(x(NaN)).toBe(2)
    expect(isNaN(y(NaN) as number)).toBe(true)
