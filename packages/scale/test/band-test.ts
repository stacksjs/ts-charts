...
it('band.paddingInner(p) coerces p to a number <= 1', () => {
  const s = scaleBand()
  expect(s.paddingInner('1.0').paddingInner()).toBe(1)
  expect(s.paddingInner('-1.0').paddingInner()).toBe(-1)
  expect(s.paddingInner('2.0').paddingInner()).toBe(1)
...
it('band.paddingOuter(p) coerces p to a number', () => {
  const s = scaleBand()
  expect(s.paddingOuter('1.0').paddingOuter()).toBe(1)
  expect(s.paddingOuter('-1.0').paddingOuter()).toBe(-1)
  expect(s.paddingOuter('2.0').paddingOuter()).toBe(2)
